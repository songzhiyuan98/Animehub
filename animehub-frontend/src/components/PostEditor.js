...
import { Box, TextField, Button, Typography, IconButton, Chip, Dialog, Avatar, Autocomplete } from '@mui/material'; // 引入 MUI 组件
...
const PostEditor = ({ onSubmit, onClose }) => {
  ...
  const handleCoverImageChange = (event) => {
    if (event.target.files && event.target.files[0]) { // 检查是否有文件
  ...
  const handleTagChange = (event, newValue) => {
    ...
    if (typeof lastTag === 'string' && lastTag.startsWith('#') && lastTag.length > 1) { // 检查标签是否有效
      const tagName = lastTag.slice(1).trim(); // 获取标签名称
      if (tagName && !availableTags.includes(tagName) && !selectedTags.includes(`#${tagName}`)) { // 检查标签是否有效
        setNewTags(prevNewTags => [...prevNewTags, tagName]); // 添加新标签
        setSelectedTags(prevTags => [...prevTags, `#${tagName}`]); // 添加到已选标签
      } else if (availableTags.includes(tagName) && !selectedTags.includes(`#${tagName}`)) { // 检查标签是否有
        setSelectedTags(prevTags => [...prevTags, `#${tagName}`]); // 添加到已选标签
      }
    } else {
      setSelectedTags(newValue.filter(tag => typeof tag === 'string' && tag.startsWith('#'))); // 过滤无效标签
    }
  };

  // 过滤标签
  const filterOptions = (options, { inputValue }) => {
    const filterValue = inputValue.startsWith('#') ? inputValue.slice(1).trim() : inputValue.trim(); // 获取过滤值
    return options
      .filter(option => 
  ...
  const handleTagDelete = (tagToDelete) => {
    setSelectedTags(prevTags => prevTags.filter(tag => tag !== tagToDelete)); // 删除已选标签
    setNewTags(prevNewTags => prevNewTags.filter(tag => `#${tag}` !== tagToDelete)); // 删除新标签
  };
  ...
  return (
    <Box sx={{ 
      ...
      <Box sx={{ mb: 2, display: 'flex', alignItems: 'center' }}>
        ...
        {coverImagePreview && (
          <Chip
            avatar={<Avatar src={coverImagePreview} alt="封面预览" />}
            label="查看封面图片"
