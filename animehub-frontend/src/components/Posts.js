...
import { Button, Dialog, Box, Typography, Avatar, Grid, Chip, IconButton, Container, CircularProgress } from '@mui/material';
import PostEditor from './PostEditor';
import axiosInstance from '../utils/axiosInstance';
import { useNavigate } from 'react-router-dom';
import { ThumbUp, ChatBubbleOutline, AccessTime, Add } from '@mui/icons-material';
...
const Post = () => {
  ...
  const fetchPosts = async (pageToFetch = page, shouldReset = false) => {
    ...
    setIsLoading(true);
    try {
      const response = await axiosInstance.get(`/posts?page=${pageToFetch}&limit=10`);
      if (response.data && Array.isArray(response.data.posts)) {
        const newPosts = response.data.posts;
        setPosts(prevPosts => shouldReset ? newPosts : [...prevPosts, ...newPosts]);
  ...
  return (
    <Box sx={{ backgroundColor: "#F2F2F2", minHeight: "100vh", color: "#333333" }}>
      ...
      >
        <PostEditor onSubmit={handleSubmitPost} onClose={() => setIsEditorOpen(false)} />
      </Dialog>
    </Box>
  );
};
